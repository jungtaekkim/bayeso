...
try:
    from bayeso.gp import gp_tensorflow as package_target
...
def test_get_optimized_kernel_typing():
    if package_target is None: # pragma: no cover
...
def test_get_optimized_kernel():
    ...
    with pytest.raises(AssertionError) as error:
        package_target.get_optimized_kernel(X, Y, prior_mu, 'se', num_iters='abc')
    with pytest.raises(AssertionError) as error:
    ...
    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(X, Y, prior_mu, 'eq', num_iters=0)
    print(hyps)

    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(X, Y, prior_mu, 'se', num_iters=0)
    print(hyps)

    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(X, Y, prior_mu, 'matern32', num_iters=0)
    print(hyps)

    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(X, Y, prior_mu, 'matern52', num_iters=0)
    print(hyps)

    cov_X_X, inv_cov_X_X, hyps = package_target.get_optimized_kernel(X, Y, prior_mu, 'matern52', num_iters=0, debug=True)
    print(hyps)
