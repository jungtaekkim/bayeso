...
def neg_log_ml(X_train: np.ndarray, Y_train: np.ndarray, hyps: np.ndarray, str_cov: str, prior_mu_train: np.ndarray,
    ...
    if use_cholesky:
        cov_X_X, lower, grad_cov_X_X = gp_common.get_kernel_cholesky(X_train, hyps, str_cov, fix_noise=fix_noise, use_gradient=use_gradient, debug=debug)
        ...
        use_gradient = False
        cov_X_X, inv_cov_X_X, grad_cov_X_X = gp_common.get_kernel_inverse(X_train, hyps, str_cov, fix_noise=fix_noise, use_gradient=use_gradient, debug=debug)
...
@utils_common.validate_types
def neg_log_pseudo_l_loocv(X_train: np.ndarray, Y_train: np.ndarray, hyps: np.ndarray, str_cov: str, prior_mu_train: np.ndarray,
    fix_noise: bool=constants.FIX_GP_NOISE,
    debug: bool=False
) -> float:
    """
    ...
    cov_X_X, inv_cov_X_X, grad_cov_X_X = gp_common.get_kernel_inverse(X_train, hyps, str_cov, fix_noise=fix_noise, debug=debug)
...
def get_optimized_kernel(X_train: np.ndarray, Y_train: np.ndarray, prior_mu: constants.TYPING_UNION_CALLABLE_NONE, str_cov: str,
    ...
    if str_modelselection_method == 'ml':
        neg_log_ml_ = lambda hyps: neg_log_ml(X_train, Y_train, hyps, str_cov, prior_mu_train, fix_noise=fix_noise, use_gradient=use_gradient, debug=debug)
    elif str_modelselection_method == 'loocv':
        neg_log_ml_ = lambda hyps: neg_log_pseudo_l_loocv(X_train, Y_train, hyps, str_cov, prior_mu_train, fix_noise=fix_noise, debug=debug)
        use_gradient = False
    ...
    if str_optimizer_method == 'BFGS':
        result_optimized = scipy.optimize.minimize(neg_log_ml_, hyps_converted, method=str_optimizer_method, jac=use_gradient, options={'disp': False})
        ...
        result_optimized = scipy.optimize.minimize(neg_log_ml_, hyps_converted, method=str_optimizer_method, bounds=bounds, jac=use_gradient, options={'disp': False})
        if debug: logger.debug('scipy message: {}'.format(result_optimized.message))
